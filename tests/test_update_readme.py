...
class TestUpdateReadme(unittest.TestCase):
    ...
    def test_calculate_current_streak_head(self):
        """Test streak head."""
        os.environ["TZ_OFFSET_HOURS"] = "6.5"
        tz_offset = datetime.timezone(datetime.timedelta(hours=6.5))
    ...
    def test_current_streak_timezone(self):
        ...
        os.environ["TZ_OFFSET_HOURS"] = "6.5"
        tz = datetime.timezone(datetime.timedelta(hours=6.5))
    ...
    def test_fetch_github_api_exception(self, mock_urlopen):
        ...
        res = _fetch_github_api("http://test")
        expected_msg = "*(Failed API request: HTTP Error 404: Not Found)*"
        self.assertTrue(res.startswith(expected_msg))
    ...
    @patch('os.environ.get')
    @patch('logging.warning')
    def test_main_no_old_readme(
            self, _mock_warning, mock_env, mock_file, mock_fetch, mock_load):
